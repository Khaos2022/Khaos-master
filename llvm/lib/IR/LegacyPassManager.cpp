...
bool FunctionPassManagerImpl::run(Function &F) {
  bool Changed = false;
  //outs() << "  FunctionPassManagerImpl::run, contains " << getNumContainedManagers() << " passes\n";
  initializeAllAnalysisInfo();
  for (unsigned Index = 0; Index < getNumContainedManagers(); ++Index) {
    //outs() << "    running FPPassManager " << getContainedManager(Index)->getPassName() << "\n";
...
bool FPPassManager::runOnFunction(Function &F) {
  ...
  //outs() << "    running FPPassManager, contains " << getNumContainedPasses() << " passes\n";
  for (unsigned Index = 0; Index < getNumContainedPasses(); ++Index) {
    FunctionPass *FP = getContainedPass(Index);
    bool LocalChanged = false;
...

    {
      ...
      //outs() << "      running FunctionPass " << FP->getPassName() << "\n";
...
MPPassManager::runOnModule(Module &M) {
  ...
  if (EmitICRemark)
    InstrCount = initSizeRemarkInfo(M, FunctionToInstrCount);
  //outs() << "  running MPPassManager, contains " << getNumContainedPasses() << " passes\n";
  for (unsigned Index = 0; Index < getNumContainedPasses(); ++Index) {
    ModulePass *MP = getContainedPass(Index);
    bool LocalChanged = false;
...

    {
      ...
      //outs() << "    running ModulePass " << MP->getPassName() << "\n";
...
bool PassManagerImpl::run(Module &M) {
  ...
  initializeAllAnalysisInfo();
  //outs() << "PassManagerImpl::run, contains " << getNumContainedManagers() << " passes\n";
  for (unsigned Index = 0; Index < getNumContainedManagers(); ++Index) {
    //outs() << "  running MPPassManager " << getContainedManager(Index)->getPassName() << "\n";
