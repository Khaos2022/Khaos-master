...
#include "../gas/aslr-guard-config.h"
...
main (int argc, char **argv)
{
  ...
  /*
  const char *code_section_names[] = {".init", ".plt", ".text",
    ".fini", "__libc_freeres_fn", "__libc_thread_freeres_fn",
    ".gcc_except_table", "__libc_subfreeres", "__libc_atexit",
    "__libc_thread_subfreeres", NULL};
  */
/*
  const char *code_section_names[] = {".init", ".plt", ".text",
    ".fini", NULL};
  size_t i_sec = 0;
  code_start = 0xFFFFFFFFFFFFFFFF, code_end = 0;
  while (code_section_names[i_sec] != NULL) {
    const char *sec_name = code_section_names[i_sec];
    asection *sec = bfd_get_section_by_name (link_info.output_bfd, sec_name);
    if (sec == NULL) {
      ++i_sec;
      continue;
    }
    if (sec->vma < code_start)
      code_start = sec->vma;
    if(sec->vma + sec->size > code_end)
      code_end = sec->vma + sec->size;
    ++i_sec;
  }

  rodata_start = 0, rodata_end = 0;
  asection *rosec = bfd_get_section_by_name (link_info.output_bfd, ".rodata");
  if (rosec != NULL) {
    rodata_start = rosec->vma;
    rodata_end = rosec->vma + rosec->size;
  }
*/
...
//***************************
// ag-note: removing unnecessary pointer encryptions for data access instructions 
// and append relocation info of them  at the end of ELF file
  ...
  size_t new_fsize = fsize +
    sizeof(Elf_Internal_Rela) * rela_code_sec.size
    + sizeof(size_t) + strlen(".rela.text");
  ...
  if (read(r_file, string, fsize) == (ssize_t)fsize) {
    ...
    for (idx = 0, rela_code_sec.cur = rela_code_sec.head;
        idx < rela_code_sec.size; ++idx) {
      // remove pointer encryption for data access instructions
      Elf_Internal_Rela * reloc = rela_code_sec.cur->reloc;
      bfd_vma addr = reloc->r_offset;
      int i;
      for (i = 0; i < code_sec_count; ++i) {
        asection *csec = &code_secs[i];
        if (addr >= csec->vma && addr < csec->vma + csec->size) {
          bfd_vma tgt = addr + 4 + *(int *)(string + (csec->filepos) + addr - csec->vma);
          char *code_enc = string + (csec->filepos) + addr - csec->vma + 4;
          // identify pointer encryption instructions based on the machine code of 
          // the first two insns of encryption routine: push %r15; mov %gs:0x100000, %r15 
          if (code_enc[4] != 0x50 || code_enc[5] != 0x51 || code_enc[48] != 0x57 || code_enc[49] != 0x65)
            break;
          // has encryption code
          int j;
          int is_code = 0;
          for (j = 0; j < code_sec_count; ++j) {
            asection *csec1 = &code_secs[j];
            if (tgt >= csec1->vma && tgt < csec1->vma + csec1->size) {
              is_code = 1;
              break;
            }
          }
          if (!is_code) {
            // use jmp 0x27 to skip pointer encryption
            code_enc[0] = 0xe9;
#ifdef USE_MAGIC_CODE
            code_enc[1] = 0x3b + 0x4e - 3 + 16;
#elif USE_NONCE_DEVRAND
            code_enc[1] = 0x5f + 0x4e - 3 + 16;
#elif USE_NONCE_RDRAND
            code_enc[1] = 0x38 + 0x4e - 3 + 16;
#else
            code_enc[1] = 0x27 + 0x4e - 3 + 16;
#endif
            code_enc[2] = 0;
            code_enc[3] = 0;
            code_enc[4] = 0;
      ...
      //  append data-access relocation data at the end of file.
      //  I did not insert these relocation entries into .rela.dyn section
      //  to avoid updating lots of data, e.g., .plt.got entries, entry point, etc.
