...
bool _in_func_syms(char *needle, struct s_asm_data *meta) {
  ...
  if (!strcmp(needle, "__reclaim_stacks")) {
      return true;
  }
  return false;
}
...
#define ASM_MPK_OPEN() do { \
    _EMITI("subq $0x10, %%rsp"); \
    _EMITI("pushq %%rax"); \
    _EMITI("pushq %%rcx"); \
    _EMITI("pushq %%rdx"); \
    _EMITI("movabs $0x0, %%rax"); \
    _EMITI("movabs $0x0, %%rcx"); \
    _EMITI("movabs $0x0, %%rdx"); \
    _EMITI(".byte 0x0f, 0x01, 0xef"); \
    _EMITI("popq %%rdx"); \
    _EMITI("popq %%rcx"); \
    _EMITI("popq %%rax"); \
    _EMITI("addq $0x10, %%rsp"); \
} while (0);

#define ASM_MPK_CLOSE() do { \
    _EMITI("subq $0x10, %%rsp"); \
    _EMITI("pushq %%rax"); \
    _EMITI("pushq %%rcx"); \
    _EMITI("pushq %%rdx"); \
    _EMITI("movabs $0x55555554, %%rax"); \
    _EMITI("movabs $0x0, %%rcx"); \
    _EMITI("movabs $0x0, %%rdx"); \
    _EMITI(".byte 0x0f, 0x01, 0xef"); \
    _EMITI("popq %%rdx"); \
...
#endif
      _EMITI("movq %%gs:0x100000(%s), %s", reg, reg);
      ASM_MPK_CLOSE();
      _EMITI("call %s", reg);
...
void process_asm(struct s_asm_data *asm_data) {
  gas_assert(asm_data);
  asm_data->new_lines = malloc((asm_data->lines_num) * 4 * sizeof(char *));
